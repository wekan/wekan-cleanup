...
def main() :
	...
	# Parameters
	month_to_keep_board_arch = 1
	month_to_keep_list_arch = 1
	month_to_keep_card_arch = 3
	mongo_user = 'admin'
	...
	time_clean_board_arch = time_start - datetime.timedelta(month_to_keep_board_arch*365/12)
	time_clean_list_arch = time_start - datetime.timedelta(month_to_keep_list_arch*365/12)
	time_clean_card_arch = time_start - datetime.timedelta(month_to_keep_card_arch*365/12)
	...
	# Clean ALL archived board (older than time_clean_board_arch)
	...
	print "[CARD_COMMENTS] Checking orphan card_comments (cardId not more in database)...."
	for card_comment in card_comments.find() :
		# Get the card
		if cards.count({"_id": card_comment['cardId']}) == 0 :
			print "[CARD_COMMENT] Deleting orphan card_comment " + str(card_comment['_id']) + " (cardId not more in database)...."
